...
import type { MoonData, TimeWindow } from '../../src/core/types.js';
...
describe('scoreNight', () => {
  it('returns not-visible when mwWindow is null', () => {
    expect(scoreNight(PALMELA, null, null, makeMoon(0))).toBe('not-visible');
  });

  it('returns not-visible when mwWindow.durationHours is 0', () => {
    expect(scoreNight(PALMELA, makeWindow(0), null, makeMoon(0))).toBe('not-visible');
  });

  it('returns not-visible when mwWindow.durationHours is negative', () => {
    expect(scoreNight(PALMELA, makeWindow(-1), null, makeMoon(0))).toBe('not-visible');
  });

  it('returns not-visible when mwWindow.durationHours < 0.5 (below minimum threshold)', () => {
    mockMoonFrac.mockReturnValue(0);
    expect(scoreNight(PALMELA, makeWindow(0.3), null, makeMoon(5))).toBe('not-visible');
  });
  ...
  it('returns best when clearHours >= 2 and moon illumination < 25', () => {
    ...
    expect(scoreNight(PALMELA, makeWindow(4), gc, makeMoon(24))).toBe('best');
  });
  ...
  it('returns best when clearHours >= 2 and moonFrac < 0.25 (dim moon irrelevant)', () => {
    ...
    expect(scoreNight(PALMELA, makeWindow(3), gc, makeMoon(80))).toBe('best');
  });

  it('returns partial when clearHours < 2 regardless of moon', () => {
    mockMoonFrac.mockReturnValue(0);
    const gc = makeWindow(1);
    expect(scoreNight(PALMELA, makeWindow(2), gc, makeMoon(5))).toBe('partial');
  });
  ...
  it('returns partial when high illumination and moon above horizon', () => {
    ...
    const gc = makeWindow(3);
    expect(scoreNight(PALMELA, makeWindow(4), gc, makeMoon(80))).toBe('partial');
  });

  it('returns partial when mwWindow.durationHours is exactly 0.5', () => {
    mockMoonFrac.mockReturnValue(0.5);
    expect(scoreNight(PALMELA, makeWindow(0.5), null, makeMoon(80))).toBe('partial');
  });

  it('boundary: clearHours exactly 2 with good moon is best', () => {
    mockMoonFrac.mockReturnValue(0);
    const gc = makeWindow(2);
    expect(scoreNight(PALMELA, makeWindow(3), gc, makeMoon(10))).toBe('best');
  });

  it('boundary: illumination 24 (below threshold) gives best', () => {
    mockMoonFrac.mockReturnValue(0.5);
    const gc = makeWindow(2);
    expect(scoreNight(PALMELA, makeWindow(3), gc, makeMoon(24))).toBe('best');
  });

  it('boundary: illumination 26 (above threshold) with high moonFrac gives partial', () => {
    mockMoonFrac.mockReturnValue(0.5);
    const gc = makeWindow(2);
    expect(scoreNight(PALMELA, makeWindow(3), gc, makeMoon(26))).toBe('partial');
  });
